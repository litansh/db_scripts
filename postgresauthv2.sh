...
#
function CONFIG_remove_ip_hba_ () {
 clear && clear && clear
 BANNER_call_allert_title_
 nniplist='/usr/lib/pgsrv/nniplist.log'
 rm -rf $nniplist
 echo -e "\n\t\t\t\t   ${menu}${menu1}Remove IPs from Permit-Trust${normal}\n"
 printf "\n\t\t\t\t          ${menu1}Trusted IPs:${normal}\n"
 BORDER_current_ip_list_
 printf "\n\t\t\t\t         ${menu1}Permitted IPs:${normal}\n"
 BORDER_current_ip_list_permitted_
 echo -e "\n\t\t\t     ${menu}Please insert IP addresses to be removed${normal} \n\t\t\t   ${menu}(like:192.168.1.1 192.168.10.0 192.168.9.0/24)${normal} \n\t\t\t\t\t   ${menu}(b = Back)${normal} \n\t\t\t  "
 local C1="$(printf "\n\t\t\t  ${bggrey}${black}${blink}=>${normal}${menu}: ${normal}")"
 read -e -p "$C1" answer
 if [ -z "$answer" ]; then
  clear && LOGO_logo_findme_ && BANNER_call_allert_title_wreqaa_ && sleep 2 && CONFIG_remove_ip_hba_
 elif [[ $answer == 'b' ]]; then
  DROPDOWN_permit_trust_
 else
  echo $answer | tr " " "\n" > $nniplist
  EDITCONF_further_edit_conf_remove_
 fi
}
#
function CONFIG_add_ip_hba_ () {
 ...
 printf "\n\t\t\t\t         ${menu1}Permitted IPs:${normal}\n"
 BORDER_current_ip_list_permitted_
 echo -e "\n\t\t\t ${menu}Please insert IP addresses to be added to permitted${normal} \n\t\t\t    ${menu}(like:192.168.1.1 192.168.10.0 192.168.9.0/24)${normal} \n\t\t\t\t\t   ${menu}(b = Back)${normal} \n\t\t\t  "
 local C1="$(printf "\n\t\t\t  ${bggrey}${black}${blink}=>${normal}${menu}: ${normal}")"
 read -e -p "$C1" answer
 if [ -z "$answer" ]; then
...
function CONFIG_add_ip_hba_trust_ () {
 ...
 printf "\n\t\t\t\t         ${menu1}Permitted IPs:${normal}\n"
 BORDER_current_ip_list_permitted_
 echo -e "\n\t\t\t ${menu}Please insert IP addresses to be added to trusted${normal} \n\t\t\t  ${menu}(like:192.168.1.1 192.168.10.0 192.168.9.0/24)${normal} \n\t\t\t\t\t   ${menu}(b = Back)${normal} \n\t\t\t  "
 local C1="$(printf "\n\t\t\t  ${bggrey}${black}${blink}=>${normal}${menu}: ${normal}")"
 read -e -p "$C1" answer
 if [ -z "$answer" ]; then
...
function CONFIG_add_ip_hba_ident_ () {
 ...
 printf "\n\t\t\t\t         ${menu1}Permitted IPs:${normal}\n"
 BORDER_current_ip_list_permitted_
 echo -e "\n\t\t\t  ${menu}Please insert IP addresses to be added to ident${normal} \n\t\t\t  ${menu}(like:192.168.1.1 192.168.10.0 192.168.9.0/24)${normal} \n\t\t\t\t\t   ${menu}(b = Back)${normal} \n\t\t\t  "
 local C1="$(printf "\n\t\t\t  ${bggrey}${black}${blink}=>${normal}${menu}: ${normal}")"
 read -e -p "$C1" answer
 if [ -z "$answer" ]; then
...
function CONFIG_add_ip_hba_peer_ () {
 ...
 printf "\n\t\t\t\t         ${menu1}Permitted IPs:${normal}\n"
 BORDER_current_ip_list_permitted_
 echo -e "\n\t\t\t  ${menu}Please insert IP addresses to be added to peer${normal} \n\t\t\t  ${menu}(like:192.168.1.1 192.168.10.0 192.168.9.0/24)${normal} \n\t\t\t\t\t   ${menu}(b = Back)${normal} \n\t\t\t  "
 local C1="$(printf "\n\t\t\t  ${bggrey}${black}${blink}=>${normal}${menu}: ${normal}")"
 read -e -p "$C1" answer
 if [ -z "$answer" ]; then
...
function DB_create_dbases_ () {
 ...
 clear && clear && clear
 BANNER_call_allert_title_
 printf "\n\n\n\t\t\t\t  ${menu1}${bggrey}${lgreen}New Database has been created: $answeruu${normal}\n"
...
echo -e "\n\n\t\t                       Press [${number}ESC${normal}] or [${number}b${normal}] to Back${normal} \n"
   #declare -a menu0_main=($OPT1 $OPT2 $OPT3 $OPT4 $OPT5 $OPT6 $OPT7 $EXIT)
   userslist='/tmp/helpme/pgsrv/userslist.log'
   echo > $userslist
   sed -i '/^$/d' $userslist
...
LOGO_logo_findme_
echo -e "\n"
local OPT1=`echo -e "Manage Access IPs"`
local OPT2=`echo -e "Manage Users"`
local OPT3=`echo -e "Manage Databases"`
if [[ $pgstatus == 'Running' ]]; then
...
LOGO_logo_findme_
echo -e "\n"
local OPT1=`echo -e "Add ${lgreen}Permitted${normal}"`
local OPT2=`echo -e "Add ${lgreen}Trusted${normal}"`
local OPT3=`echo -e "Add ${lgreen}Ident${normal}"`
local OPT4=`echo -e "Add ${lgreen}Peer${normal}"`
