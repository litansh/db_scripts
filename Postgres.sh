...
if [[ ! -f $permittedlist ]]; then
 ...
 check=`cat $permittedlist | grep 'ident' | wc -l`
 listaa=`cat $permittedlist | sed 's/md5/Password needed All Users/g' | sed 's/ident/Password Less Specific Users/g' | awk '{print $1, "|", $2, $3, "|", $4, $5}' | column -t`
 longstr=`(echo -e "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" | awk '{if(length>x){x=length;y=$0}}END{print y}')`
 longlen=39
...
if [[ ! -f $trustedlist ]]; then
 ...
 listaa=`cat $trustedlist | sed 's/trust/Password Less All allowed/g' | awk '{print $1, "|", "Password Less", "|", "All allowed"}' | column -t`
 longstr=`(echo -e "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" | awk '{if(length>x){x=length;y=$0}}END{print y}')`
 longlen=39
 edge=$(echo -e "$longstr" | sed 's/./-/g')
 echo -e "${grey}\t\t  +$edge---+${normal}"
...
function BORDER_show_db_ (){
 PGPASSWORD=myPassword psql -U postgres -c "\l" | awk '{print $1, $3}' | grep -v 'List' | grep -v 'Name' | grep -v '-' | grep -v '|' | grep -v '(' | sed '/^$/d' | column -t > $dblist
 listaa=`cat $dblist | awk '{print $1, "| Owned by", $2, "| Allowed from", $3}' | column -t`
...
function BORDER_show_removed_ (){
 ...
 while IFS= read -r line; do
  strlen=${#line}
  echo -e -n "${grey}\t\t   |${normal} \t\t\t${lgreen}     $line${normal}${normal}"
  gap=$((longlen - strlen))
  if [ "$gap" > 0 ]; then
   for i in $(seq 1 $gap); do echo -n " "; done
   echo -e "${grey} |${normal}"
  ...
  if [[ $line =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
   ...
   if [[ $checkbit > 0 ]] && [[ $checkip == 0 ]]; then
    ...
    if [[ $checksub == 0 ]]; then
     ...
     sed -i "/^# IPv6.*/i aaa" $hbaconf
     sed -i "s|aaa|$linea|" $hbaconf
     echo "${line}/24 trust" >> $trustedlist
     ...
     sed -i "/^# IPv6.*/i aaa" $hbaconf
     sed -i "s|aaa|$linea|" $hbaconf
     echo "${line}/32 trust" >> $trustedlist
...
function CONFIG_add_ip_hba_ () {
 ...
 printf "\t\t\t\t           ${menu}Trusted IPs${normal} ${number}$trustedcount${normal}\n"
 printf "\n\t\t        ${number}IP Address           Password            Allowed Users${normal}\n"
 BORDER_current_ip_list_
 printf "\n\t\t\t\t          ${menu}Permitted IPs${normal} ${number}$permittedcount${normal}\n"
 printf "\n\t\t        ${number}IP Address           Password              Allowed Users${normal}\n"
...
function DROPDOWN_remove_ip_ () {
  ...
  local EXIT=`echo -e "Back"`
  LOGO_logo_findme_
echo -e "\n\n\t\t           ${menu}Press [${number}ENTER${normal}${menu}] to Remove or [${number}ESC${menu}] / [${number}b${menu}] to go back\n"
...
if [[ -s $tmp ]]; then
 tmpview=`cat $tmp | tr '\n' ','`
 echo -e "\t\t\t        ${menu}IPs have been ${lgreen}removed${menu} from permitted${normal}"
 BORDER_show_removed_
fi
rm -rf $tmp
unset tmpview
echo -e "\n"
...
printf "\n\t\t\t\t           ${menu}Trusted IPs${normal} ${number}$trustedcount${normal}\n"
printf "\n\t\t        ${number}IP Address           Password            Allowed Users${normal}\n"
BORDER_current_ip_list_
printf "\n\t\t\t\t          ${menu}Permitted IPs${normal} ${number}$permittedcount${normal}\n"
printf "\n\t\t        ${number}IP Address           Password              Allowed Users${normal}\n"
